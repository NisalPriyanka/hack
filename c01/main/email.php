<!DOCTYPE html>
<html>
<head>
<link rel="stylesheet" href="boost/css/bootstrap.min.css" type="text/css" />
<script type="text/javascript" src="boost/js/bootstrap.min.js"></script>
</head>
<body>
<?php
	if(isset($_POST['sbmt']))
	{
		if($_POST['mail']=='december20@stalker.com')
		{
			echo "<script> window.open('winning.php','_TOP') </script>";	
		}
	}

?>

<div class="container-fluid">

  <div class="row">
		<div class="col-md-12"><h2>Central Communication Panel</h2></div>
  </div>
 
  <div class="row">
  <div class="col-md-6">
 <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST">
 	<div class="form-group">
    	<label for="mail">E-Mail</label>
        <input type="email" name="mail" class="form-control" />
    </div> 
    <div class="form-group">
    	<label for="msg">Message</label>
        <textarea class="form-control" row="10">Write Your Message Here..</textarea>
    </div> 
    <input type="submit" name="sbmt" value="Send" class="btn btn-primary" />
 </form>
   </div>
   <div class="col-md-6"></div>
 
  </div>
 
</div>

</body>
</html>